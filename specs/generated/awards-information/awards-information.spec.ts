...
const testData = TestDataLoader.load('awards-information', 'awards-information');
...
test.describe.serial('Awards Information page', () => {
  let page: Page;
  ...
  test('A logged-in member sees the coming-soon placeholder', { tag: ['@auto', '@smoke'] }, async () => {
    // Open awards-information page
    await page.goto('/awards-information', { waitUntil: 'load' });
    ...
    // Assert award information nav is visible
    await expect(page.locator('header >> role=link[name="Award Information"]')).toBeVisible();
