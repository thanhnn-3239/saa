...
**Why it's wrong:**
`brainstorm` is CTO-level trade-off analysis — it debates *which approach* to take, not *does the work*.
It will not produce a redesigned page. It will produce a list of options and their trade-offs.

**Use `brainstorm` when:** "Should I use PostgreSQL or MongoDB?", "Should I build a monolith or microservices?", "Which auth strategy should I pick?"
...
## ❌ Using `/tkm:find-skill` to route within the installed kit

**Wrong scenario:**
> "Which skill should I use to fix my bug?" → `/tkm:find-skill`

**Right choice:** `/tkm:help` (which is this skill) or directly `/tkm:fix-bug`

**Why it's wrong:**
`find-skill` searches an EXTERNAL registry (skills.sh) for NEW skills to install.
It does not know about or route within the installed Takumi kit.

**Use `find-skill` when:** "Is there a skill for analyzing Terraform costs?" (may not be installed)
