...
- **Notifications API layer** (`lib/notifications/`, `app/api/notifications/`): `GET /api/notifications` (paginated feed, cursor pagination), `GET /api/notifications/unread-count`, `POST /api/notifications/mark-read` (single or all). Full error handling: 401/400/404/500.
- **`useMarkRead` hook** (`lib/notifications/use-mark-read.ts`): returns `{ markOne, markAll, isPending }` with optimistic TanStack Query cache updates for both the feed and unread-count queries.
